...
suite('stream<T> lowers', () => {
    ...
    describe('async', () => {
        ...
        // count instead of one item per canonical write (see lann/jco#15).
        //
        ...
        // little-endian to match the lift (see lann/jco#8)
        test.concurrent('flags (more than 8 members)', async () => {
            const instance = await getInstance();
            assert.instanceOf(instance['jco:test-components/stream-lower-async'].readStreamValuesWideFlags, AsyncFunction);

            const allUnset = {
                f00: false, f01: false, f02: false, f03: false,
                f04: false, f05: false, f06: false, f07: false,
                f08: false, f09: false, f10: false, f11: false,
            };
            const vals = [
