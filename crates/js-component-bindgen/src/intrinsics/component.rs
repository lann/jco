            ...
            Self::ComponentAsyncStateClass => {
                ...
                output.push_str(&format!(
                    ...
                    class {component_async_state_class} {{
                        ...
                        // an interleaved slice of the same component corrupts (see
                        // lann/jco#30, lann/jco#47, lann/jco#48).
                        //
                        ...
                        exclusiveRelease(taskID) {{
                            ...
                            if (this.#lockHolderTaskID !== taskID) {{
                                ...
                                // release now leaves the hold intact (see lann/jco#30).
                        ...
                        tick() {{
                            ...
                            for (const taskID of resumableTasks) {{
                               const meta = this.#suspendedTasksByTaskID.get(taskID);
                                ...
                                // NOTE: single-flight resumption: resume at most one task per
                                ...
                                // resumed. Resuming multiple suspended tasks in one synchronous
                                // cascade interleaves their register save/restore windows
                                // ([restoreA, restoreB, resumeA, resumeB]), re-entering wasm
                                // with the register naming the wrong task (see lann/jco#31,
                                // and the 'known residual' of the JSPI current-task register
                                // fix); with concurrent task lifetimes per component this
                                // corrupts guest context-local storage (see lann/jco#40).
                                return {component_async_state_class}.TickResult.RESUMED;
                            }}
